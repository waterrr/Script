...
Check_Libsodium_ver(){
	...
	[[ -z 1.0.18-RELEASE ]] && Libsodiumr_ver=${Libsodiumr_ver_backup}
	echo -e "${Info} libsodium 最新版本为 ${Green_font_prefix}1.0.18-RELEASE${Font_color_suffix} !"
}
Install_Libsodium(){
	...
	if [[ ${release} == "centos" ]]; then
		...
		echo -e "${Info} 下载..."
		wget  --no-check-certificate -N "https://github.com/jedisct1/libsodium/releases/download/1.0.18-RELEASE/libsodium-1.0.18-RELEASE.tar.gz"
		echo -e "${Info} 解压..."
		tar -xzf libsodium-1.0.18-RELEASE.tar.gz && cd libsodium-1.0.18-RELEASE
		echo -e "${Info} 编译安装..."
		...
		echo -e "${Info} 下载..."
		wget  --no-check-certificate -N "https://github.com/jedisct1/libsodium/archive/1.0.18-RELEASE.tar.gz"
		echo -e "${Info} 解压..."
		tar -xzf 1.0.18-RELEASE.tar.gz && cd libsodium-1.0.18-RELEASE
		echo -e "${Info} 编译安装..."
		./configure --disable-maintainer-mode && make -j2 && make install
	fi
	ldconfig
	cd .. && rm -rf libsodium-1.0.18-RELEASE.tar.gz && rm -rf libsodium-1.0.18-RELEASE
