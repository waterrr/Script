...
Install_Libsodium_Water(){
	...
	if [[ ${release} == "centos" ]]; then
		...
		yum -y groupinstall "Development Tools"
		echo -e "${Info} 下载..."
		wget  --no-check-certificate -N "http://cdn.gksec.com/libsodium-1.0.18.tar.gz"
		echo -e "${Info} 解压..."
...
if [[ "${action}" == "clearall" ]]; then
  ...
  ${Green_font_prefix}44.${Font_color_suffix} 安装 libsodium(chacha20) 走水鸽CDN下载安装[稳]
————————————
